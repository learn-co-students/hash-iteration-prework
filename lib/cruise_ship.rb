...
passengers.each do |suite, name|
  #We iterate through the hash using #each.
  ...
  if suite == :suite_a && name.start_with?("A")
    #Inside our iteration, we use an if statement combined with the && ("and") boolean operator
    #to check if we have the right suite and if the person in that suite has a name that
    #begins with the letter "A".
    winner = name
    #If that condition returns true, we've found our winner!
    #We set their name equal to the winner variable and end our iteration.
  end
end
